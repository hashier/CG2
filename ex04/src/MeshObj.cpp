...
MeshObj::MeshObj() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    mMaxBounds[i] = -std::numeric_limits<float>::max(); // thanks to Jan for pointing out FLT_MIN being the smallest POSITIVE float possible //
  }
}
