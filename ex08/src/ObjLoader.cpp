...
void ObjLoader::computeTangentSpace(std::vector<Vertex> &vertexList, const std::vector<unsigned int> &indexList) {
  ...
  for (unsigned int i = 0; i < (indexList.size() - 2); i += 3) {
    ...
    GLfloat Q1[3] = {v1.position[0] - v0.position[0],
                     v1.position[1] - v0.position[1],
                     v1.position[2] - v0.position[2]};                 
    GLfloat Q2[3] = {v2.position[0] - v0.position[0],
    ...
    if (det != 0) {
      ...
      for (unsigned int j = 0; j < 3; ++j) {
        GLfloat T = (Q1[j] * dv2 - Q2[j] * dv1) * invDet;
        v0.tangent[j] += T;
        v1.tangent[j] += T;
        v2.tangent[j] += T;
      }
  ...
  for (unsigned int i = 0; i < vertexList.size(); ++i) {
    ...
    GLfloat NdotT = (v.normal[0] * v.tangent[0])
